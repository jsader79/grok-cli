  ...
  async processUserMessage(message: string): Promise<ChatEntry[]> {
    ...
    try {
      ...
      if (toolRounds >= maxToolRounds) {
        const error = new MaxToolRoundsError(maxToolRounds);
    ...
    try {
      ...
      switch (toolCall.function.name) {
        ...
        default:
          ...
          const notFoundError = new ToolNotFoundError(toolCall.function.name);
          return {
            success: false,
            error: formatErrorForUser(notFoundError),
          };
      }
    } catch (error: any) {
      ...
      return {
        success: false,
        error: formatErrorForUser(execError),
      };
    }
  }
  ...
  private async executeMCPTool(toolCall: GrokToolCall): Promise<ToolResult> {
    ...
    try {
      ...
      if (result.isError) {
        ...
        const serverName = toolCall.function.name.split('__')[1] || 'unknown';
        const toolName = toolCall.function.name.split('__').slice(2).join('__') || toolCall.function.name;
      ...
      return {
        success: false,
        error: formatErrorForUser(mcpError),
      };
    }
  }
